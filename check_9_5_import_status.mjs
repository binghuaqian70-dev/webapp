...
async function main() {
  ...
  try {
    ...
    if (fs.existsSync('9_5_import_stats.json')) {
      const stats = JSON.parse(fs.readFileSync('9_5_import_stats.json', 'utf8'));
      ...
      // 计算实际进度
      const expectedTotal = 174893 + 29571; // 初始数据库记录 + 预期导入记录
      const actualImported = currentStats.total - stats.totalRecords;
      console.log('\n📊 实际状态:');
      console.log(`   数据库记录增长: +${actualImported.toLocaleString()} 条`);
      console.log(`   预期总记录数: ${expectedTotal.toLocaleString()}`);
      console.log(`   完成进度: ${(actualImported / 29571 * 100).toFixed(2)}%`);
      
    } else {
      console.log('⚠️ 未找到导入统计文件');
    }
    
    // 检查进度文件
    if (fs.existsSync('9_5_import_progress.json')) {
      const progress = JSON.parse(fs.readFileSync('9_5_import_progress.json', 'utf8'));
    ...
    if (fs.existsSync('9_5_import_progress.json')) {
      ...
      console.log('      命令: ./start_9_5_import.sh');
    } else {
