...
const STATS_FILE = './10_30_import_stats.json';
const PROGRESS_FILE = './10_30_import_progress.json';
const LOG_FILE = './10_30_import.log';
...
function checkProcessStatus() {
  ...
  try {
    // 检查导入进程
    const result = execSync('ps aux | grep -E "(optimized_batch_import|check_10_30_import_status)" | grep -v grep', { encoding: 'utf-8' }).trim();
...
function provideStatusAdvice(stats, progress, fileStatus) {
  ...
  if (!stats && !progress) {
    ...
    console.log('   💡 运行以下命令启动导入: ./start_10_30_import.sh');
    return;
  }

  if (stats?.status === 'completed') {
    console.log('   🎉 导入已完成！');
    console.log('   📊 查看完整统计: cat 10_30_import_stats.json');
    return;
  }
  ...
  if (stats?.status === 'running' || progress) {
    ...
    console.log('   📋 查看实时日志: tail -f 10_30_import.log');
    return;
  }
...
// 主函数
async function main() {
  console.log('🔍 10.30数据汇总表批量导入状态检查');
  console.log('============================================================');
