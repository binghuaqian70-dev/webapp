#!/bin/bash
###############################################################################
# 8.11数据汇总表批量导入系统验证脚本（优化版）
# 功能：检查Node.js环境、脚本文件、AI Drive目标文件
###############################################################################

echo "============================================"
echo "  8.11批量导入系统验证（优化版）"
echo "============================================"
echo ""
...
for script in "start_8_11_optimized_import.sh" "check_8_11_optimized_import.sh" "verify_8_11_optimized_system.sh"; do
...
echo ""
echo "   📊 找到 $FILES_FOUND / 6 个文件，总大小: ${TOTAL_SIZE_MB}MB"
echo ""
...
if [ $CHECKS_PASSED -eq $CHECKS_TOTAL ]; then
    echo "✅ 系统验证通过！可以开始导入"
    echo ""
    echo "🚀 启动命令: ./start_8_11_optimized_import.sh"
    exit 0
elif [ $FILES_FOUND -eq 6 ]; then
