...
const TARGET_FILES = [
  '10.28数据汇总表-utf8_part_01.csv',
  '10.28数据汇总表-utf8_part_02.csv',
  '10.28数据汇总表-utf8_part_03.csv',
  '10.28数据汇总表-utf8_part_04.csv',
  '10.28数据汇总表-utf8_part_05.csv',
  '10.28数据汇总表-utf8_part_06.csv'
];

const STATS_FILE = './10_28_import_stats.json';
const PROGRESS_FILE = './10_28_import_progress.json';
const LOG_FILE = './10_28_import.log';
    ...
    if (fs.existsSync(STATS_FILE)) {
      ...
      if (stats.fileResults && Object.keys(stats.fileResults).length > 0) {
        ...
        Object.entries(stats.fileResults).forEach(([file, result]) => {
          ...
          const records = result.importedRecords || result.imported || 0;
...
function checkProcessStatus() {
  ...
  try {
    // 检查导入进程
    const result = execSync('ps aux | grep -E "(optimized_batch_import|check_10_28_import_status)" | grep -v grep', { encoding: 'utf-8' }).trim();
...
function provideStatusAdvice(stats, progress, fileStatus) {
  ...
  if (!stats && !progress) {
    ...
    console.log('   💡 运行以下命令启动导入: ./start_10_28_import.sh');
    return;
  }

  if (stats?.status === 'completed') {
    console.log('   🎉 导入已完成！');
    console.log('   📊 查看完整统计: cat 10_28_import_stats.json');
    return;
  }
  ...
  if (stats?.status === 'running' || progress) {
    ...
    console.log('   📋 查看实时日志: tail -f 10_28_import.log');
    return;
  }
...
// 主函数
async function main() {
  console.log('🔍 10.28数据汇总表批量导入状态检查');
  console.log('============================================================');
