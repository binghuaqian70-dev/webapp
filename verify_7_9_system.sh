#!/bin/bash
# 7.9数据汇总表导入系统准备就绪检查脚本

echo "🔍 7.9数据汇总表导入系统准备就绪检查"
echo "========================================"
echo ""
...
if [ -f "import_7_9_batch.mjs" ]; then
    check_pass "主导入脚本存在"
    if [ -x "import_7_9_batch.mjs" ]; then
        check_pass "主导入脚本可执行"
    else
...
if [ -f "start_7_9_import.sh" ]; then
    check_pass "启动脚本存在"
    if [ -x "start_7_9_import.sh" ]; then
        check_pass "启动脚本可执行"
    else
        check_fail "启动脚本不可执行"
    fi
...
if [ -f "check_7_9_import.sh" ]; then
    check_pass "进度检查脚本存在"
    if [ -x "check_7_9_import.sh" ]; then
...
for i in {1..20}; do
    FILE="/mnt/aidrive/7.9数据汇总表-utf8_part${i}.csv"
    if [ -f "$FILE" ]; then
        ((FOUND_FILES++))
...
if [ $FOUND_FILES -eq 0 ]; then
    ...
    echo "   💡 文件命名: 7.9数据汇总表-utf8_part1.csv ~ 7.9数据汇总表-utf8_part20.csv"
...
if pgrep -f "import_7_9_batch.mjs" > /dev/null; then
    ...
    echo "   💡 如需重新开始，请先停止现有进程: pkill -f import_7_9_batch.mjs"
else
...
if [ -f "7_9_import_progress.json" ]; then
...
if [ $FAILED_CHECKS -eq 0 ]; then
    if [ $FOUND_FILES -gt 0 ]; then
        ...
        echo ""
        echo "📝 下一步操作:"
        echo "   1. 运行启动脚本: ./start_7_9_import.sh"
        echo "   2. 查看实时进度: ./check_7_9_import.sh"
        echo "   3. 监控日志: tail -f 7_9_import.log"
    else
...
echo ""
echo "📋 参考文档:"
echo "   - 快速参考: cat 7.9导入快速参考.md"
echo "   - 完整指南: cat 7.9导入完整指南.md"
echo ""
