 ...
 * 支持AI Drive中7.21数据汇总表-utf8_part_1.csv到part_12.csv的12个分割文件批量导入
...
const TARGET_FILE_PREFIX = '7.21数据汇总表-utf8_part';
// 12个文件：7.21数据汇总表-utf8_part_1.csv 到 7.21数据汇总表-utf8_part_12.csv
const TARGET_FILES = Array.from({ length: 12 }, (_, i) => {
  const num = i + 1;
  return `7.21数据汇总表-utf8_part_${num}.csv`;
});

// 优化配置 - 针对7.21数据汇总表多文件批量导入调整（12个文件）
...
const PROGRESS_FILE = './7_21_import_progress.json'; // 7.21进度文件路径
const LOG_FILE = './7_21_import.log'; // 详细日志文件
const STATS_FILE = './7_21_import_stats.json'; // 统计数据文件
...
async function main() {
  // 初始化日志
  log('🚀 7.21数据汇总表批量导入系统启动');
  log(`📍 AI Drive: ${AI_DRIVE_PATH}`);
  log(`📍 生产环境: ${PRODUCTION_URL}`);
  log(`🎯 目标文件: ${TARGET_FILES.length}个分割文件 (part_1 - part_12)`);
