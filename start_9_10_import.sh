...
# 打印带颜色的消息
print_status() {
    echo -e "${BLUE}🔍 9.10数据汇总表导入状态检查${NC}"
    echo "=================================="
}
...
start() {
    ...
    if is_running; then
        ...
        print_info "   ./start_9_10_import.sh status    # 查看详细状态"
        print_info "   ./start_9_10_import.sh logs      # 查看实时日志"
        print_info "   node check_9_10_import_status.mjs # 详细进度报告"
    else
...
status() {
    ...
    # 运行状态检查脚本
    if [ -f "check_9_10_import_status.mjs" ]; then
        node check_9_10_import_status.mjs
    else
        print_warning "状态检查脚本不存在: check_9_10_import_status.mjs"
    fi
}
