...
# 文件路径
SCRIPT_PATH="./optimized_batch_import.mjs"
LOG_FILE="./10_15_import.log"
STATS_FILE="./10_15_import_stats.json"
PROGRESS_FILE="./10_15_import_progress.json"
AI_DRIVE_PATH="/mnt/aidrive"
TARGET_FILE="10.15数据汇总表-utf8.csv"

echo -e "${BLUE}🚀 10.15数据汇总表导入启动脚本${NC}"
echo -e "${BLUE}================================================${NC}"
...
echo -e "   📊 文件大小: ${FILE_SIZE_KB}KB (${FILE_SIZE} bytes)"
...
# 检查是否有正在运行的进程
RUNNING_PROCESSES=$(ps aux | grep "optimized_batch_import.mjs" | grep -v grep | wc -l)
...
echo -e "   检查导入状态: ${BLUE}node check_10_15_import_status.mjs${NC}"
echo -e "   查看进程状态: ${BLUE}ps aux | grep optimized_batch_import.mjs${NC}"
