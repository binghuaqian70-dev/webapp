#!/usr/bin/env node
/**
 * 10.22数据汇总表批量导入状态检查脚本
 * 实时监控10个分割文件的导入进度和数据库状态
 */
...
const STATS_FILE = './10_22_import_stats.json';
const PROGRESS_FILE = './10_22_import_progress.json';
const LOG_FILE = './10_22_import.log';
...
async function checkProcessStatus() {
  try {
    ...
    try {
      // 检查是否有相关导入进程在运行
      const psOutput = execSync('ps aux | grep -E "(optimized_batch_import|10.22)" | grep -v grep', { encoding: 'utf8', timeout: 5000 });
...
async function main() {
  console.log('🔍 10.22数据汇总表批量导入状态检查');
