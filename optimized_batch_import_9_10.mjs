#!/usr/bin/env node
/**
 * 优化的批量导入脚本 - 9.10数据汇总表导入版本 (多文件批量导入)
 * 后台运行、进度统计、分阶段导入策略，支持6位小数价格精度
 * 支持AI Drive中9.10数据汇总表-utf8_part_1.csv到part_8.csv的8个分割文件批量导入
...
const AI_DRIVE_PATH = '/tmp/9_10_import_cache';
const TARGET_FILE_PREFIX = '9.10数据汇总表-utf8_part_';
// 15个文件：9.10数据汇总表-utf8_part_1.csv 到 9.10数据汇总表-utf8_part_8.csv (8个文件)
const TARGET_FILES = Array.from({ length: 8 }, (_, i) => {
  const num = i + 1;
  return `9.10数据汇总表-utf8_part_${num}.csv`;
});

// 优化配置 - 针对9.10数据汇总表多文件批量导入调整（8个文件）
