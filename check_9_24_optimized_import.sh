#!/bin/bash
###############################################################################
# 9.24数据汇总表批量导入进度监控脚本（优化版）
# 功能：实时查看optimized_batch_import.mjs的导入进度和统计信息
###############################################################################

STATS_FILE="/home/user/webapp/9_24_import_stats.json"
LOG_FILE="/home/user/webapp/9_24_import.log"

# 清屏
clear

echo "============================================"
echo "  9.24数据汇总表批量导入进度监控（优化版）"
echo "============================================"
echo ""
...
# 计算进度百分比（使用awk代替bc，避免依赖缺失）
if [ "$TOTAL_FILES" -gt 0 ]; then
    FILE_PROGRESS=$(awk -v a="$PROCESSED_FILES" -v b="$TOTAL_FILES" 'BEGIN{printf "%.2f", (a/b)*100}')
else
    FILE_PROGRESS="0.00"
fi

if [ "$TOTAL_CHUNKS" -gt 0 ]; then
    CHUNK_PROGRESS=$(awk -v a="$PROCESSED_CHUNKS" -v b="$TOTAL_CHUNKS" 'BEGIN{printf "%.2f", (a/b)*100}')
else
    CHUNK_PROGRESS="0.00"
fi

if [ "$TOTAL_RECORDS" -gt 0 ]; then
    RECORD_PROGRESS=$(awk -v a="$IMPORTED_RECORDS" -v b="$TOTAL_RECORDS" 'BEGIN{printf "%.2f", (a/b)*100}')
else
...
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "💡 刷新命令: watch -n 2 ./check_9_24_optimized_import.sh"
