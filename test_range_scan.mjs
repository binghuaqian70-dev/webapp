#!/usr/bin/env node
/**
 * 测试9.2汇总表文件范围扫描功能 (part_044 到 part_100)
 */
...
async function main() {
  ...
  try {
    ...
    if (files.length === 0) {
      console.log('⚠️ 未找到指定范围内的9.2汇总表文件');
      return;
    }
    ...
    if (files.length > 5) {
      console.log('   ...');
      console.log('📁 后5个文件详情:');
      files.slice(-5).forEach((file, index) => {
        const globalIndex = files.length - 4 + index;
        console.log(`   ${globalIndex}. ${file.filename}`);
        console.log(`      公司: ${file.company}`);
        console.log(`      大小: ${formatFileSize(file.size)}`);
        console.log(`      记录数: ${formatNumber(file.estimatedRecords)} 条`);
      });
