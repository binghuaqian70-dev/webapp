...
const PROGRESS_FILE = './9_10_import_progress.json';
const LOG_FILE = './9_10_import.log';
const STATS_FILE = './9_10_import_stats.json';

// 9.10数据配置
const START_PART = 1;
const END_PART = 20;
const EXPECTED_FILES = 20;
const EXPECTED_RECORDS = 100000; // 预期记录数基线
...
function checkAiDriveFiles() {
  try {
    ...
    const csvFiles = files.filter(file => {
      return file.endsWith('.csv') && file.includes('9.10数据汇总表');
    }).sort();
    ...
    csvFiles.forEach(file => {
      ...
      try {
        ...
        estimatedRecords += Math.floor(stats.size / 118);
      } catch (error) {
        // 忽略单个文件的错误
      }
    });
...
async function main() {
  ...
  if (fileStatus.error) {
    ...
    console.log(`✅ 找到 ${fileStatus.found}/${fileStatus.expected} 个9.10数据汇总表文件`);
  ...
  if (token) {
    ...
    // 计算导入增长（假设基线）
    const baselineRecords = stats?.totalRecords || 343944; // 使用统计数据中的基线或默认值
    ...
    console.log(`🎯 9.10数据导入进度: ${progressPercentage.toFixed(1)}% (预期${formatNumber(EXPECTED_RECORDS)}条)`);
  } else {
  ...
  console.log(`📝 建议: 使用 ./start_9_10_import.sh status 查看详细状态`);
