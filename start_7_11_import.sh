...
LOG_FILE="7_11_import.log"
STATS_FILE="7_11_import_stats.json"
PROGRESS_FILE="7_11_import_progress.json"
NOHUP_LOG="7_11_import_nohup.log"

cd "$SCRIPT_DIR" || exit 1

echo "🚀 7.11数据汇总表批量导入系统"
echo "================================"
echo ""
...
const TARGET_FILES = Array.from({ length: 20 }, (_, i) => {
  return \`7.11数据汇总表-utf8_part\${i + 1}.csv\`;
});
...
if (foundCount === 0) {
  ...
  console.log('💡 文件命名格式: 7.11数据汇总表-utf8_part1.csv ~ 7.11数据汇总表-utf8_part20.csv');
  process.exit(1);
}
" || exit 1
