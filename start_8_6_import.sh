...
IMPORT_SCRIPT="import_8_6_batch.mjs"
LOG_FILE="8_6_import.log"
STATS_FILE="8_6_import_stats.json"
PROGRESS_FILE="8_6_import_progress.json"
NOHUP_LOG="8_6_import_nohup.log"
...
const TARGET_FILES = Array.from({ length: 16 }, (_, i) => {
  return \`8.6数据汇总表-utf8_part_\${i + 1}.csv\`;
});
...
if (foundCount === 0) {
  ...
  console.log('💡 文件命名格式: 8.6数据汇总表-utf8_part_1.csv ~ 8.6数据汇总表-utf8_part_16.csv');
  process.exit(1);
}
" || exit 1
