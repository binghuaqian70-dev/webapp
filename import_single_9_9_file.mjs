...
// 分割CSV内容为小块 - 针对9.9数据优化
function splitCsvContent(csvContent) {
  const lines = csvContent.split('\n').filter(line => line.trim());
  const header = lines[0];
  const dataLines = lines.slice(1);
  
  // 9.9数据文件根据实际行数动态调整块大小
  const totalLines = dataLines.length;
  let chunkSize = 80;
  
  if (totalLines > 2000) {
    chunkSize = 80; // 大文件使用适中块提高效率
  } else if (totalLines > 1000) {
    chunkSize = 100; // 中等文件
  } else if (totalLines > 500) {
    chunkSize = 150; // 较小文件可以更大块
  } else {
    chunkSize = 200; // 小文件使用大块提高效率
  }
...
async function importSingleFile(filename, token) {
  try {
    ...
    if (filename.startsWith('/')) {
      ...
      // 如果文件不存在，尝试添加9.9数据汇总表前缀
      if (!fs.existsSync(filePath) && !filename.includes('9.9数据汇总表')) {
        // 尝试匹配part编号格式
        const partMatch = filename.match(/(part_\d{2})/);
        if (partMatch) {
          const possibleName = `9.9数据汇总表-utf8_${partMatch[1]}.csv`;
    ...
    // 估算记录数
    const estimatedRecords = Math.floor(stats.size / 120);
...
async function main() {
  log('🚀 9.9数据汇总表单文件导入脚本启动');
  
  // 获取命令行参数
  const filename = process.argv[2];
  
  if (!filename) {
    console.error('❌ 使用方法: node import_single_9_9_file.mjs <文件名>');
    console.error('');
    console.error('示例:');
    console.error('  node import_single_9_9_file.mjs part_01');
    console.error('  node import_single_9_9_file.mjs 9.9数据汇总表-utf8_part_01.csv');
    console.error('  node import_single_9_9_file.mjs /mnt/aidrive/9.9数据汇总表-utf8_part_01.csv');
    process.exit(1);
  }
