#!/usr/bin/env node
/**
 * 优化的批量导入脚本 - 9.24数据汇总表导入版本 (多文件批量导入)
 * 后台运行、进度统计、分阶段导入策略，支持6位小数价格精度
 * 支持AI Drive中9.24数据汇总表-utf8_part_1.csv到part_8.csv的8个分割文件批量导入
...
const AI_DRIVE_PATH = '/tmp/9_24_import_cache';
const TARGET_FILE_PREFIX = '9.24数据汇总表-utf8_part_';
// 8个文件：9.24数据汇总表-utf8_part_1.csv 到 9.24数据汇总表-utf8_part_8.csv (注意：无前导零，使用下划线)
const TARGET_FILES = Array.from({ length: 8 }, (_, i) => {
  const num = i + 1;
  return `9.24数据汇总表-utf8_part_${num}.csv`;
});

// 优化配置 - 针对9.24数据汇总表多文件批量导入调整（8个文件）
...
const PROGRESS_FILE = './9_24_import_progress.json'; // 9.24进度文件路径
const LOG_FILE = './9_24_import.log'; // 详细日志文件
const STATS_FILE = './9_24_import_stats.json'; // 统计数据文件
...
async function main() {
  // 初始化日志
  log('🚀 9.24数据汇总表批量导入系统启动');
  log(`📍 AI Drive: ${AI_DRIVE_PATH}`);
  log(`📍 生产环境: ${PRODUCTION_URL}`);
  log(`🎯 目标文件: ${TARGET_FILES.length}个分割文件 (part_1 - part_8)`);
