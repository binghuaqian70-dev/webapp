...
// 指定导入范围
const START_PART = 61;  // part_061
const END_PART = 100;   // part_100
...
function getTargetFiles() {
  try {
    ...
    // 筛选指定范围的文件：part_061 到 part_100
    // 文件名格式：9.1汇总表-utf8_part_061.csv
    const targetFiles = csvFiles.filter(filename => {
      const match = filename.match(/9\.1汇总表-utf8_part_(\d+)\.csv/);
      if (match) {
    ...
    targetFiles.sort((a, b) => {
      const partA = a.match(/9\.1汇总表-utf8_part_(\d+)\.csv/);
      const partB = b.match(/9\.1汇总表-utf8_part_(\d+)\.csv/);
...
async function main() {
  console.log('🚀 优化批量导入 - 指定范围版本 (part_061 - part_100)');
  console.log(`📍 AI Drive: ${AI_DRIVE_PATH}`);
  console.log(`📍 生产环境: ${PRODUCTION_URL}`);
  console.log(`📊 导入范围: part_${START_PART.toString().padStart(3, '0')} 到 part_${END_PART.toString().padStart(3, '0')}`);
  ...
  try {
    ...
    // 获取目标文件列表
    console.log('📂 扫描目标范围CSV文件...');
    const files = getTargetFiles();
    console.log(`📋 找到 ${files.length} 个目标文件 (预期40个文件)\n`);

    if (files.length === 0) {
      console.log('⚠️ 未找到目标范围的CSV文件');
      return;
    }
    ...
    console.log('\n' + '='.repeat(80));
    console.log('📊 指定范围导入完成总结 (part_061 - part_100)');
    console.log('='.repeat(80));
    console.log(`📊 目标文件范围: part_${START_PART.toString().padStart(3, '0')} 到 part_${END_PART.toString().padStart(3, '0')}`);
    console.log(`📁 找到目标文件: ${files.length} 个`);
