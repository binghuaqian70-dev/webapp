#!/usr/bin/env node
/**
 * 优化的批量导入脚本 - 6.26数据汇总表导入版本 (多文件批量导入)
 * 后台运行、进度统计、分阶段导入策略，支持6位小数价格精度
 * 支持AI Drive中6.26数据汇总表-utf8_part1.csv到part5.csv的5个分割文件批量导入
...
const TARGET_FILE_PREFIX = '6.26数据汇总表-utf8_part';
// 5个文件：6.26数据汇总表-utf8_part1.csv 到 6.26数据汇总表-utf8_part5.csv (注意：无前导零)
const TARGET_FILES = Array.from({ length: 5 }, (_, i) => {
  return `6.26数据汇总表-utf8_part${i + 1}.csv`;
});

// 优化配置 - 针对6.26数据汇总表多文件批量导入调整（5个文件）
...
const PROGRESS_FILE = './6_26_import_progress.json'; // 6.26进度文件路径
const LOG_FILE = './6_26_import.log'; // 详细日志文件
const STATS_FILE = './6_26_import_stats.json'; // 统计数据文件
