#!/usr/bin/env node
/**
 * 优化的批量导入脚本 - 6.30数据汇总表导入版本 (多文件批量导入)
 * 后台运行、进度统计、分阶段导入策略，支持6位小数价格精度
 * 支持AI Drive中6.30数据汇总表-utf8_part01.csv到part12.csv的12个分割文件批量导入
...
const TARGET_FILE_PREFIX = '6.30数据汇总表-utf8_part';
// 12个文件：6.30数据汇总表-utf8_part01.csv 到 6.30数据汇总表-utf8_part12.csv (注意：有前导零)
const TARGET_FILES = Array.from({ length: 12 }, (_, i) => {
  const num = String(i + 1).padStart(2, '0');
  return `6.30数据汇总表-utf8_part${num}.csv`;
});

// 优化配置 - 针对6.30数据汇总表多文件批量导入调整（12个文件）
...
const PROGRESS_FILE = './6_30_import_progress.json'; // 6.30进度文件路径
const LOG_FILE = './6_30_import.log'; // 详细日志文件
const STATS_FILE = './6_30_import_stats.json'; // 统计数据文件
...
async function main() {
  // 初始化日志
  log('🚀 6.30数据汇总表批量导入系统启动');
  log(`📍 AI Drive: ${AI_DRIVE_PATH}`);
  log(`📍 生产环境: ${PRODUCTION_URL}`);
  log(`🎯 目标文件: ${TARGET_FILES.length}个分割文件 (part01 - part12)`);
