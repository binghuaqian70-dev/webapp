#!/usr/bin/env node
/**
 * 优化的批量导入脚本 - 6.2数据汇总表导入版本 (多文件批量导入)
 * 后台运行、进度统计、分阶段导入策略，支持6位小数价格精度
 * 支持AI Drive中6.2数据汇总表-utf8_part01.csv到part40.csv的40个分割文件批量导入
...
const TARGET_FILE_PREFIX = '6.2数据汇总表-utf8_part';
// 40个文件：6.2数据汇总表-utf8_part01.csv 到 6.2数据汇总表-utf8_part40.csv
const TARGET_FILES = Array.from({ length: 40 }, (_, i) => {
  const num = String(i + 1).padStart(2, '0');
  return `6.2数据汇总表-utf8_part${num}.csv`;
});

// 优化配置 - 针对6.2数据汇总表多文件批量导入调整（40个文件）
...
const PROGRESS_FILE = './6_2_import_progress.json'; // 6.2进度文件路径
const LOG_FILE = './6_2_import.log'; // 详细日志文件
const STATS_FILE = './6_2_import_stats.json'; // 统计数据文件
