...
IMPORT_SCRIPT="import_8_11_batch.mjs"
LOG_FILE="8_11_import.log"
STATS_FILE="8_11_import_stats.json"
PROGRESS_FILE="8_11_import_progress.json"
NOHUP_LOG="8_11_import_nohup.log"

cd "$SCRIPT_DIR" || exit 1

echo "🚀 8.11数据汇总表批量导入系统"
echo "================================"
echo ""
...
const TARGET_FILES = Array.from({ length: 6 }, (_, i) => {
  return \`8.11数据汇总表-utf8_part_\${i + 1}.csv\`;
});
...
if (foundCount === 0) {
  ...
  console.log('💡 文件命名格式: 8.11数据汇总表-utf8_part_1.csv ~ 8.11数据汇总表-utf8_part_6.csv');
  process.exit(1);
}
" || exit 1
