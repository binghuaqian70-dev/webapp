...
if (!targetFile) {
  console.log('🔧 9.13数据汇总表单文件导入工具');
  console.log('=====================================');
  console.log('');
  console.log('用法: node import_single_9_13_file.mjs <文件名或part编号>');
  console.log('');
  console.log('示例:');
  console.log('  node import_single_9_13_file.mjs part1');
  console.log('  node import_single_9_13_file.mjs part5');
  console.log('  node import_single_9_13_file.mjs 9.13数据汇总表-part3.csv');
  console.log('');
  console.log('支持的文件范围: part1 到 part10');
  process.exit(1);
}
...
function findTargetFile(target) {
  try {
    ...
    console.log(`🔍 AI Drive中找到 ${csvFiles.length} 个9.13数据汇总表文件`);
    ...
    if (target.endsWith('.csv')) {
      ...
      // 如果目标是part编号 (如 part1, part5)
      const partMatch = target.match(/part(\d{1,2})/i);
      if (partMatch) {
        const partNum = parseInt(partMatch[1]);
        const expectedFile = `9.13数据汇总表-part${partNum}.csv`;
    ...
    console.log('\n📋 可用的9.13数据汇总表文件:');
    csvFiles.sort((a, b) => {
      const partA = a.match(/part(\d{1,2})/);
      const partB = b.match(/part(\d{1,2})/);
      if (partA && partB) {
        return parseInt(partA[1]) - parseInt(partB[1]);
      }
      return a.localeCompare(b);
    }).forEach((file, index) => {
      const partMatch = file.match(/part(\d{1,2})/);
