...
LOG_FILE="10_28_import.log"
STATS_FILE="10_28_import_stats.json"
PROGRESS_FILE="10_28_import_progress.json"
STATUS_SCRIPT="check_10_28_import_status.mjs"
...
# 工具函数
print_header() {
    echo -e "${BLUE}🚀 10.28数据汇总表批量导入系统${NC}"
    echo -e "${BLUE}==================================${NC}"
    echo ""
}
...
get_user_confirmation() {
    echo ""
    print_section "🚨 重要提醒:"
    echo -e "${YELLOW}   • 本次导入将处理 6 个分割文件${NC}"
    echo -e "${YELLOW}   • 导入过程可能需要较短时间 (预计8-15分钟)${NC}"
    echo -e "${YELLOW}   • 导入过程中请勿关闭终端或中断程序${NC}"
    echo -e "${YELLOW}   • 支持断点续传，意外中断后可重新运行${NC}"
    echo -e "${YELLOW}   • 建议在稳定的网络环境下运行${NC}"
    echo -e "${YELLOW}   • 6个文件比之前20个文件处理更快${NC}"
    echo ""
...
start_import_process() {
    ...
    print_section "🚀 启动10.28数据汇总表批量导入..."
    print_section "=================================="
    echo ""
